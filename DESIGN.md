...
- Session Picker：每次 room PTY 未启动时自动弹出，含 CLI 选择（紫色）+ 模型下拉选择；三角色时切换三列布局；Claude/Kimi/Codex 显示历史会话列表；spawn 成功后立即同步 `currentRoom.{role}SessionId`，确保下次 Session Picker 默认选中正确会话
