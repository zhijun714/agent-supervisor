...
- Session Picker：每次 room PTY 未启动时自动弹出，含 CLI 选择（紫色）+ 模型下拉选择；三角色时切换三列布局；Claude/Kimi/Codex 显示历史会话列表
...
| POST | `/rooms/:id/spawn` | 启动 Room 的 PTY（含 CLI/模型/sessionId 选择） |
...
├── rooms.json         # Room 配置持久化（git 忽略，含 archCli/devCli/qaCli/archSessionId/devSessionId/qaSessionId 字段）
├── package.json
├── prompts/
