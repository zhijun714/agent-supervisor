...
spawn 时，`buildMemoryContext` 检查 `devDir` 下是否存在 `CONTEXT.md`（领域术语表）：若无，则向 PA 和 Dev 的 prompt 注入简短提醒，引导其用 `domain-modeling` 技能起一份种子 `CONTEXT.md`，再继续设计/命名，以防术语漂移；若已有则技能自读，无需额外注入。
...
Supervisor 通过一键脚本把精选外部工程技能 vendoring 进 Claude Code 用户级 skills 目录（`~/.claude/skills/`），三个角色共享，无需改 spawn 逻辑。

**机制：`scripts/install-skills.sh` + `scripts/skills-config.json`**
- 从 Matt Pocock skills 和 Superpowers 两个仓库按 ALLOWLIST 精确拣选 14 个技能（8 核心 + 6 可选）
- pin commit（`skills-config.json` 记录 SHA），确保可复现；`--update` 模式展示 diff、`--update --apply` 升级并更新 config
- 安全 marker（`.managed-by-supervisor`）：已有目录若无 marker 则跳过，不覆盖用户自管理的技能
- 名称冲突检测：ALLOWLIST 内若两个技能 frontmatter name 重复，退出报错

**治理铁律：**
- 不装任何 SessionStart hook，不改 CLAUDE.md / settings.json
- 只装"节点技能"（纯 markdown，无运行时 SDK 依赖），不装任何 orchestrator（gstack/GSD 整体排除）
- 细节见 `ai-docs/external-skills-adoption.md`
...
- **tab 常驻与分组（服务端持久化）**：Room 数据模型中三个字段控制 tab 状态：`opened`（房间是否在左侧打开/常驻）、`pinned`（新义：是否置于「关注」上组）、`order`（拖拽排序键，组内升序）。已打开的 tab 按 pinned 上组 / 普通下组渲染，中间一条横线分隔；各组内按 `order` 升序排列。纯拖拽操作：组内拖动排序、拖过横线切换分组，落定后乐观更新本地 `latestRooms` 防止 3 s 轮询回弹，并批量 POST `/tabs/layout`（`[{id,pinned,order}]`，一次写防竞态）持久化。打开房间 → PUT `{opened:true}`；手动关闭 tab → POST `/rooms/:id/close`：`opened=false` 并杀掉该房间 arch/dev/qa 的 PTY；上次激活的 tab 用 `localStorage['sup-active-room']` 记忆。**迁移**：服务启动时一次性把旧 `pinned`（旧义=已打开）迁为 `opened`，新 `pinned` 从 false 起，`order` 按旧 `updatedAt` 降序赋值；迁移完立即落盘，重启幂等。`openTabs(Map)` 是 iframe 生命周期的唯一真源，拖拽排序仅改渲染顺序、不销毁/重建 iframe。
...
- **xterm 右边界修复**：已移除 `.term-wrap` 的 `padding-right:6px`。该 padding 不被 FitAddon 感知（`getComputedStyle` 在 `box-sizing:border-box` 下返回 border-box 宽），反使 `.xterm` 实际宽度收窄 6px，导致最右列 CJK 字符贴边显示；移除后 `.xterm` 获得完整宽度，右缘 CJK 正常留边。
- **终端渲染健壮性（robustFit）**：首次 fit 等 `document.fonts.ready` 后再执行，避免 fallback 字体量出偏小 charWidth 导致 cols 算大、PTY 内容散开；切换到前台时经 `room_activated` postMessage 触发重 fit（2×rAF 确保 display:block 生效后再量）；ResizeObserver 50 ms 防抖防止窗口缩放时并发 fit 竞态；自愈重试（B）：检测 `.xterm-rows` container 的 `letterSpacing`（= `cellWidth − widthCache("W")`，正常 ≈ 0，坏态 ≈ 3.18），`|ds| > 0.5` 即判 charWidth 量测异常，有界重试最多 3 次（120/250/500 ms 递增），每次 `handleCharSizeChanged()` 强制清 widthCache + 重算 defaultSpacing，直到恢复正常或达上限放弃。解决冷加载/切换/缩放下的字距错乱、半高、cols 偏大内容散开等偶发问题。
- **WebSocket scheme 跟随页面协议**：WS 地址由 `location.protocol` 推导（`https:` → `wss:`，`http:` → `ws:`），支持经 HTTPS 反代访问（如 `supervisor.example.com`），避免浏览器 Mixed Content 拦截；对任意层数反代均成立，前提是各层代理正确转发 WebSocket Upgrade 头。
...
- **codebase-design 审查**：验收时用深模块词汇（模块深度/删除测试）评估接口质量，驳回浅模块；同时驳回 scope-creep（夹带重构、未要求的防御代码、过早抽象、超出需求的 API 面）
...
| PUT | `/rooms/:id` | 更新 Room（name/dirs/silent/model/`opened`/`pinned`/`order` 字段；dirs 传空串即停用该角色） |
| POST | `/rooms/:id/close` | 关闭 tab：`opened=false` 并杀掉该房间 arch/dev/qa 的 PTY |
| POST | `/tabs/layout` | 批量更新 tab 分组与排序（`[{id, pinned, order}]`，一次写防竞态） |
