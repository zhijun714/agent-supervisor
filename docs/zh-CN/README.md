...
> 多角色 AI 编程协作工具 — 架构师、开发工程师和 QA 在真实 PTY 终端里协作。
...
现在很多工具让一个 AI 扮演整个团队——但有三件事，单会话再强也做不到：

- **上下文隔离，QA 才真对抗** — QA 是独立进程，从没看过实现思路，才能不被"自己写的代码"带偏。一个会话里扮演 QA 的 AI 早看过自己的实现，假装不了。这是结构差异，不是提示词能补的。
- **长存活·并行·可单独干预** — Dev 和 QA 是长期并行的对等进程：能同时干活、能在 Dev 跑一半时插紧急纠正而不打断 QA、能给 Dev 单独换模型各自扛限流。不是用完即弃的子任务。
- **看得见·可接管的控制面** — 房间、终端回放、记忆面板、飞书 relay 一应俱全；每个角色在干什么实时可见，随时介入。工具是没有控制面的技能包。

> 工具让每个角色"个人更强"，独立进程提供它们之间的"真分离"——两者叠加，不互相替代。
...
- **左侧房间 tab** — 多房间以左侧标签常驻，点击瞬时切换且后台保活（不重连终端）；打开的房间刷新页面或服务重启后自动恢复，手动关闭才移除并断开后端终端。已打开的 tab 按一条横线分两组：**关注**（置顶）/ **普通**，纯拖拽操作：组内拖动排序、拖过横线切换分组；分组与排序服务端持久化
